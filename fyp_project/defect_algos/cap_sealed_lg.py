# -*- coding: utf-8 -*-
"""cap_sealed_lg_houghlines.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1u5vrpuvgPaymCvGyvrPhAZ-dipL9aFXJ
...
def cap_sealed_lg(img_name):
    ...
    cap_lid = cap_only[0:140,200:]
    # plt.imshow(cap_lid)

    blur_lid = cv2.medianBlur(cap_lid,5) 
    # plt.imshow(blur_lid)

    cap_left = cap_lid[:,0:300]
    cap_right = cap_lid[:,600:]

    # plt.subplot(1,2,1),plt.imshow(cap_left)
    # plt.subplot(1,2,2),plt.imshow(cap_right)
    # cv2.imshow("c1",cap_left)
    # cv2.waitKey(0)
    # cv2.imshow("c2",cap_right)
    # cv2.waitKey(0)
    ...
    # cv2.imshow("c3",edgesl)
    # cv2.waitKey(0)
    # cv2.imshow("c4",edgesr)
    # cv2.waitKey(0)
    ...
    # cv2.imshow("c5",closingl)
    # cv2.waitKey(0)
    # cv2.imshow("c6",closingr)
    # cv2.waitKey(0)
    ...
    lines = cv2.HoughLines(closingl, 1, np.pi / 180, 100)
    ...
    lines = cv2.HoughLines(closingr, 1, np.pi / 180, 80)
    rhos = []
    if lines is not None:
        for i in range(0, len(lines)):
            rho = lines[i][0][0]
